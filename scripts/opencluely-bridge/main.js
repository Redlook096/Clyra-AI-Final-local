...
class ApplicationController {
  ...
  setupStealth() {
    // Stealth rejected — run as honest OpenCluely + Clyra helper
  ...
  async onAppReady() {
    ...
    // Honest branding — no Terminal disguise / stealth
    app.setName("OpenCluely");
    process.title = "OpenCluely";
    ...
    logger.info("Application starting", {
      ...
      stack: "clyra-api + moondream vision",
    });
    ...
    try {
      ...
      // Skip Gemini onboarding — Clyra API + local Moondream are the stack
      let status;
      try {
        ...
        this.isFirstRun = false;
        logger.info("First-run status", {
          ...this.firstRunManager.getStatus(),
          skippedGeminiOnboarding: true,
        });
      } catch (e) {
      ...
      try {
        if (app && typeof app.setName === "function") app.setName("OpenCluely");
        process.title = "OpenCluely";
      } catch (_) {
        /* optional */
      }
