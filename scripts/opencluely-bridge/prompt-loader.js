...
- Avoid unnecessary verbosity; focus on correctness, clarity, and efficiency.`;
  ...
  getAvailableSkills() {
    ...
    // Prefer dynamic discovery from prompts/ — always include general + dsa
    const discovered = [];
    try {
      const files = fs.readdirSync(this.promptsDir);
      for (const file of files) {
        if (file.endsWith('.md')) discovered.push(path.basename(file, '.md'));
      }
    } catch (_) {
      /* ignore */
    }
    const skills = Array.from(new Set(['general', 'dsa', ...discovered]));
    return skills;
  }
