...
class MainWindowUI {
    ...
    resizeWindowToContent() {
        ...
        setTimeout(() => {
            const shell = document.getElementById('ocShell') || document.querySelector('.command-tab');
            if (shell && window.electronAPI && window.electronAPI.resizeWindow) {
                ...
                if (this.shortcutsPopover && this.shortcutsPopover.classList.contains('is-open')) {
                    ...
                    width = Math.max(width, Math.ceil(popRect.right - rect.left + 8));
                }
