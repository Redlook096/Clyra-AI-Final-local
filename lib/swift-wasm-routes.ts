...
export function registerSwiftWasmRoutes(app: Application) {
  ...
  app.get("/api/swift-wasm/projects/:projectId/preview", (req, res) => {
    ...
    const kind = /budget|spend|finance|transaction|account|expense/.test(sourceWords) ? "finance"
    ...
    res.type("html").send(renderSwiftSourcePreview({ title, texts, buttons, kind }));
  });
}
