...
Clyra re-clones [OpenCluely](https://github.com/TechyCSR/OpenCluely) (MIT) as a **reference** under `references/OpenCluely` and ports its **glass overlay UI** as a general desktop helper — not an interview stealth tool.

## What we keep from OpenCluely

| Idea | Destination |
| --- | --- |
| Dark glass command tab + chat panel look | `electron/companion.html`, web `ScreenCompanionWorkspace` |
...
- Gemini/Azure/Whisper installer stack (Clyra already has STT/TTS + RapidOCR)
