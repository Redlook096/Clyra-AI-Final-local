...
export function Sidebar({
  ...
  return (
    <motion.aside
      ...
      className={cn("relative flex h-full min-w-0 shrink-0 flex-col overflow-hidden border-r border-black/[0.06] bg-[#F7F7F6]", collapsed && "pointer-events-none border-r-0")}>
      ...
      {!collapsed ? <div className="cc-scroll mt-2 min-h-0 flex-1 overflow-y-auto px-1.5 pb-2" onClick={() => setMenu(null)}>
        {filteredProjects.length === 0 ? <div className="px-1.5 py-1.5 text-[11.5px] leading-[1.45] text-[#96989D]">No matching projects.</div> : filteredProjects.map((project) => {
          ...
          return <div key={project.id} className="group/project relative mb-2.5 last:mb-1">
            <div role="button" tabIndex={0} onClick={() => { setExpanded((old) => { const next = new Set(old); next.has(project.id) ? next.delete(project.id) : next.add(project.id); return next; }); onSelectProject(project.id); }} onKeyDown={(event) => event.key === "Enter" && onSelectProject(project.id)} className={cn("flex h-[30px] w-full items-center gap-1 rounded-[7px] px-1.5 text-left transition-colors duration-100", selected ? "bg-black/[0.045]" : "hover:bg-black/[0.03]")}>
            ...
            {isExpanded ? <div className="ml-[17px] border-l border-black/[0.055] py-0.5 pl-1">{projectThreads.length ? projectThreads.map((thread) => {
              ...
              return <div key={thread.id} role="button" tabIndex={0} onClick={() => onSelectChat(project.id, thread.id)} onContextMenu={(e) => { e.preventDefault(); openChatMenu(e, project.id, thread.id); }} className={cn("group/chat relative flex h-[29px] w-full items-center gap-1 rounded-[7px] pl-2 pr-1 text-left transition-colors duration-100", threadSelected ? "bg-black/[0.045]" : "hover:bg-black/[0.03]")}>{chatEditing ? <input ref={editRef} value={editing.value} onChange={(e) => setEditing({ ...editing, value: e.target.value })} onBlur={commitEdit} onKeyDown={(e) => { if (e.key === "Enter") commitEdit(); if (e.key === "Escape") setEditing(null); }} onClick={(e) => e.stopPropagation()} className="min-w-0 flex-1 rounded-[4px] bg-white px-1 text-[12px] text-[#303136] outline-none ring-1 ring-[#3977F6]/35" /> : <><span className="min-w-0 flex-1 truncate text-[12px] text-[#5B5D62]">{highlight(thread.title || "New chat")}</span>{threadSelected ? <span className="h-[4px] w-[4px] rounded-full bg-[#3977F6]" /> : null}</>}<button type="button" aria-label={`Chat menu for ${thread.title}`} onClick={(e) => openChatMenu(e, project.id, thread.id)} className="invisible absolute right-1 flex h-5 w-5 items-center justify-center rounded-[5px] bg-[#F7F7F6] text-[#76787D] opacity-0 transition-opacity duration-100 hover:bg-black/[0.05] group-hover/chat:visible group-hover/chat:opacity-100"><MoreHorizontal className="h-3 w-3" /></button></div>;
            }) : <button type="button" onClick={() => onNewChat(project.id)} className="flex h-[27px] w-full items-center px-2 text-[11.5px] text-[#93959A] transition-colors hover:text-[#57595E]">New chat</button>}</div> : null}
          </div>;
        })}
