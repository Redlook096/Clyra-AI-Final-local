...
* You generated an HTML page, built SPA, or other previewable web artifact →
    command="show_preview", path=<workspace-relative path, e.g. dist/index.html>
    (Opens the **Browser** tab with a live iframe preview — prefer the dev-server
    URL when running; for Vite builds use `npm run build -- --base=./` first.)
